...
ScreenGui.Name = "KHAN_GUI_3RD_PERSON"
ScreenGui.ResetOnSpawn = false
ScreenGui.Parent = LocalPlayer:WaitForChild("PlayerGui")

-------------------------------------------------------------------------------
-- VISUAL FOV CIRCLE (ตรึงอยู่กลางจอถาวรตามแนว crosshair ในมุมมองบุคคลที่ 3)
-------------------------------------------------------------------------------
...
MinButton.MouseButton1Click:Connect(function()
    ...
    local targetSize = Settings.Collapsed and UDim2.new(0, 250, 0, 40) or UDim2.new(0, 250, 0, 390)
...
CreateSlider("FOV Radius", 10, 300, Settings.FOV, 4, function(v) Settings.FOV = v end)
CreateSlider("Lock Speed (1=Fast)", 1, 10, Settings.Smoothness, 5, function(v) Settings.Smoothness = v end)
CreateSlider("WalkSpeed", 20, 24, Settings.WalkSpeed, 6, function(v) Settings.WalkSpeed = v end)

-------------------------------------------------------------------------------
-- CORE LOGIC (3rd Person Aimbot Engine)
-------------------------------------------------------------------------------
RunService.RenderStepped:Connect(function()
    ...
    -- 4. 3rd Person Optimized Aimbot Logic
    ...
    if Settings.AimbotEnabled then
        ...
        if closest and closest.Character and closest.Character:FindFirstChild("Head") then
            ...
            -- คำนวณหาทิศทางจากกล้องไปหาศัตรูในมุมมองบุคคลที่ 3 โดยตรงเพื่อความนิ่ง
            local cameraPosition = Camera.CFrame.Position
            local lookAtCFrame = CFrame.new(cameraPosition, targetPosition)
            
            -- ปรับระดับความเร็วในการดีดเป้า
            local lockAmount = 1 / math.max(Settings.Smoothness, 1)
            
            if Settings.Smoothness == 1 then
                -- บังคับกล้องหมุนไปหาเป้าหมายโดยตรงทันทีแบบ 3 มิติแท้
                Camera.CFrame = lookAtCFrame
                
                -- ล็อกให้ตัวละครหันหน้าตรงตามทิศทางกล้องไปด้วยเพื่อแก้บั๊กมุมมองบุคคลที่ 3 ลากเป้าไม่อยู่
                ...
                -- ล็อกแบบสมูทลื่นไหลชดเชยค่า Offset ของมุมกล้อง 3rd Person
